...
// A. كود Vertex Shader
// وظيفته: تحديد موقع رؤوس المستطيل في الفضاء
const char* vertexShaderSource = "#version 330 core\n"
"layout (location = 0) in vec3 aPos;\n" // نستقبل البيانات في الموقع 0
"uniform mat4 model;\n" //تعريف مصفوفة 4*4
"void main()\n"
"{\n"
"   gl_Position = model * vec4(aPos, 1.0);\n" // نحول النقطة من vec3 إلى vec4
"}\0";
...
"void main()\n"
"{\n"
"   FragColor = vec4(0.6f, 0.8f, 0.9f, 1.0f);\n" 
"}\n\0";
...
int main()
{
    ...
    // إنشاء كائن النافذة
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "LearnOpenGL - First Rectangle", NULL, NULL);
    if (window == NULL)
    {
    ...
    float vertices[] = {
        -0.8f, -0.5f, 0.5f,// يسار أسفل
        -0.8f,  0.5f, 0.5f,// يسار أعلى
         0.8f, -0.5f, 0.5f, // يمين أسفل
         0.8f,  0.5f, 0.5f  // يمين أعلى
    };
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glm::mat4 model = glm::mat4(1.0f); // مصفوفة هوية

        float time = glfwGetTime(); // يحدث دوران مستمر

        // دوران حول X
        model = glm::rotate(model, time, glm::vec3(1.0f, 0.0f, 0.0f));

        // دوران حول Y
        model = glm::rotate(model, time * 0.7f, glm::vec3(0.0f, 1.0f, 0.0f));

        // دوران حول Z
        model = glm::rotate(model, time * 1.3f, glm::vec3(0.0f, 0.0f, 1.0f));

        unsigned int modelLoc = glGetUniformLocation(shaderProgram, "model"); 
        glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model)); // (نعطيه عنوان البيانات (الشيدر يستلم المصفوفة

        glBindVertexArray(VAO); // تفعيل كائن المستطيل
        glDrawArrays(GL_TRIANGLE_STRIP, 0, 4); // ارسم مستطيل من 4 رؤوس
